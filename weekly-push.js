...
async function main() {
  ...
  try {
    ...
    // 2. Format WeChat Message
    const pushTitle = `🎯 ${year}年第${getWeekNumber(now)}周 | 辅导员思政热点雷达`;
    
    // Clean bad XML character sequences
    content = content
      .replace(/[\u0000-\u0008\u000B-\u000C\u000E-\u001F]/g, '')
      .replace(/&(?!amp;|lt;|gt;|quot;|apos;)/g, '&amp;');

    const pushSummary = `本周《青年思政热点雷达》已出炉！点击查看最新最接地气的选题灵感与思政切入大纲。`;
    ...
    const pushResponse = await fetch('https://wxpusher.zjiecode.com/api/send/message', {
      ...
      body: JSON.stringify({
        ...
        content: content,
        summary: pushSummary,
        contentType: 3, // 3 means Markdown (beautiful presentation hosted on WxPusher servers)
        uids: WX_UIDS
      })
    });
